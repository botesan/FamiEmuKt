...
//println("mapper4 irq reload. irqCounter=$irqCounter, irqCounterTemp=$irqCounterTemp") // TODO: デバッグ
        ...
        when (address) {
            ...
            in 0xA000..0xBFFF -> {
                if (address and 0x01 == 0x00) {
                    ...
                    if (mirroring != Mirroring.FourScreen) {
                        mirroring = if (value.toInt() and 0x01 == 0x00) Mirroring.Vertical else Mirroring.Horizontal
                    }
                } else {
                    /* https://www.nesdev.org/wiki/MMC3
            ...
            in 0xE000..0xFFFF -> {
                if (address and 0x01 == 0x00) {
                    ...
                    isIRQEnabled = false
                    interrupter.requestOffIRQ() // TODO: 合ってるっぽい？
                } else {
    ...
    override fun readCHR(address: Int): UByte {
        ...
        val bankNo = if (isCHRMode0) {
            ...
            when (address) {
                in 0x0000..0x03FF -> bankNoR[2]
                in 0x0400..0x07FF -> bankNoR[3]
                in 0x0800..0x0BFF -> bankNoR[4]
                in 0x0C00..0x0FFF -> bankNoR[5]
                in 0x1000..0x13FF -> bankNoR[0] and 0xFE
                in 0x1400..0x17FF -> (bankNoR[0] and 0xFE) + 1
                in 0x1800..0x1BFF -> bankNoR[1] and 0xFE
                in 0x1C00..0x1FFF -> (bankNoR[1] and 0xFE) + 1
                else -> error("Illegal address. ${address.toString(radix = 16).padStart(length = 4, padChar = '0')}")
            }
        }
